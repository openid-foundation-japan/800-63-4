...
このセクションでは、なりすましと虚偽・詐称表現の脅威を中心に説明する。インフラストラクチャの脅威は、従来のコンピュータセキュリティコントロール（例：侵入防御、記録保持、独立監査）によって考慮されており、このドキュメントの範囲外であるためである。セキュリティ対策の詳細については、[[[SP800-53]](sec11_references.ja.md#ref-SP800-53) _Recommended Security and Privacy Controls for Federal Information Systems and Organizations_ を参照すること。
